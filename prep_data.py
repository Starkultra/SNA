...
table = create_table(connection, cursor, query="CREATE TABLE IF NOT EXISTS drug_action(cid TEXT, target TEXT, ki text, ic50 text, kd text);")
if table:
    print("Table Found/Created")
    print("Inserting Values...")
    insert_logic = 'INSERT INTO drug_action values("{}", "{}", "{}", "{}", "{}");'
    transact = partial(transaction_builder, connection=connection, cursor=cursor)
    with open("./dataset/BindingDB_All.tsv", "r", buffering=1000) as f:
        for row in f:
            if row_counter == 0:
                row_counter += 1
            else:
                data = row.split("\t")
                insert_statement = insert_logic.format(data[28], data[6], data[8], data[9], data[10])
